...
public class MemberManager {
	 ...
	 * 根据session获取当前登录的用户
	 * */
	public final static MemberModel getCurrentUser(final HttpServletRequest request){
		sessionTimeout();
		if (sessionMap.containsKey(request.getSession())) {
			return (MemberModel) sessionMap.get(request.getSession()).get(USER);
		}
		return null;
	}
	 ...
	 * */
	public final static void addUser(final HttpServletRequest request,MemberModel model){
		sessionTimeout();
		if (sessionMap.containsKey(request.getSession())){
